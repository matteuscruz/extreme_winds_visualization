"""
Motor de dados e gráficos do painel.

Este módulo não desenha nada por conta própria: ele carrega os artefatos,
calcula as métricas e devolve figuras Plotly prontas. Quem monta a tela é
`app.py` (casca + navegação), `narrativa.py` (as seções que contam a história)
e `explorador.py` (o painel de consulta técnica).

Extraído de `app.py` sem alteração de lógica — a separação é de
responsabilidade, não de comportamento.
"""
...
# Nome de cada configuração na tela. Mesma tabela de `apuracao.NOME_ARM`.
ARM_LABELS = {
    "original": "Base",
...
NENHUMA = "(nenhuma)"
def rotulo_area(cluster_id) -> str:
    return f"Área {cluster_id}"
...
def periodos_dos_experimentos() -> dict:
    """Recortes de treino/validação/teste LIDOS dos `run_meta.json`.

    Antes isto era uma constante digitada à mão no código
    ...
    chaves = {"treino": "train_slice", "validacao": "val_slice", "teste": "test_slice"}
    for meta_path in sorted(ABLATION_DIR.glob("*/*/run_meta.json")):
        try:
            meta = json.loads(meta_path.read_text())
        except (OSError, json.JSONDecodeError):
            continue
        nome = f"{meta_path.parent.parent.name}/{meta_path.parent.name}"
        for rotulo, chave in chaves.items():
            fatia = meta.get(chave)
            if not fatia:
                continue
            contagem[rotulo].setdefault(tuple(fatia), []).append(nome)

    resultado: dict = {"divergentes": []}
    for rotulo, vistos in contagem.items():
        if not vistos:
            resultado[rotulo] = None
            continue
        maioria = max(vistos, key=lambda k: len(vistos[k]))
        resultado[rotulo] = maioria
        for fatia, nomes in vistos.items():
            if fatia != maioria:
                resultado["divergentes"].append((rotulo, fatia, nomes))
    return resultado


PERIODOS = periodos_dos_experimentos()
TRAIN_PERIOD = PERIODOS["treino"] or ("—", "—")
VAL_PERIOD = PERIODOS["validacao"] or ("—", "—")
TEST_PERIOD = PERIODOS["teste"] or ("—", "—")
        ...
        for arm in ABLATION_ARMS:
            ...
            if (d / "results.parquet").exists():
                combos.append({
                    ...
                    "label": f"{PIPELINE_LABELS.get(pipeline, pipeline)} / {ARM_LABELS.get(arm, arm)}",
                })
    return combos
...
def _geom_for_cluster_choice(cluster_choice: str | None):
    ...
    `cluster_choice` para None ou para a seleção de todas as áreas.
    ...
    if not cluster_choice or cluster_choice == TODAS_AS_AREAS:
        geom = basin_union
    else:
...
def build_best_per_cluster_bars(
    ...
    cluster_labels = [rotulo_area(c) for c in winners_df["cluster_id"]]
...
def build_lazy_top5_per_arm(
    ...
    for c in combos:
        ...
        if cluster_choice != TODAS_AS_AREAS:
            chosen = cluster_choice.removeprefix("Área ")
    ...
    for c in combos:
        ...
        if not shown_observed:
            fig.add_trace(go.Histogram(
                x=df["y_true"], name="Observado (INMET)",
                marker_color="#52514e", opacity=0.35, histnorm="probability density",
            ))
            shown_observed = True
        fig.add_trace(go.Histogram(
            x=df["y_pred"], name=f"{c['label']} (previsto)",
...
def build_residual_timeseries(estacao: str | None, mlp_preds_df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        name="Resíduo (MLP)", line={"color": PIPELINE_COLORS["mlp"], "width": 1.3},
    ))
...
def build_interp_map(
    ...
    if not stations_only:
        ...
        fig.add_trace(go.Scattermap(
            ...
            hoverinfo="skip", showlegend=False, name="Cobertura de dado",
        ))
        fig.add_trace(go.Scattermap(
...
def build_ablation_scatter_with_ols(
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="Observado: %{x:.2f}<br>Previsto: %{y:.2f}<extra></extra>",
        name=f"{pipeline}/{arm}",
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        name=f"Reta ajustada (R²={r2:.3f})",
    ))
    fig.update_layout(
        title=(
            f"Observado × Previsto — {PIPELINE_LABELS.get(pipeline, pipeline)} / {arm}<br>"
            f"<sup>viés={metrics['bias']:+.2f} · REQM={metrics['rmse']:.2f} · "
    ...
    # A diagonal 1:1 só é legível se os dois eixos tiverem a mesma escala.
    # `scaleanchor` faria isso, mas trava a proporção: dentro de um container
    # que o Streamlit monta escondido (aba, expander), a figura nasce sem
    # largura utilizável e aparece encolhida. Fixar o mesmo `range` nos dois
    # eixos dá o mesmo efeito visual sem depender da largura do container.
    margem = (hi - lo) * 0.02 if hi > lo else 1.0
    fig.update_xaxes(range=[lo - margem, hi + margem])
    fig.update_yaxes(range=[lo - margem, hi + margem])
    return fig
...
def build_lstm_loss_curve(histories: dict, cluster_id, season: str) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=epochs, y=loss, mode="lines", name="erro no treino",
                              line={"color": "#2a78d6"}))
    if val_loss:
        fig.add_trace(go.Scatter(x=list(range(1, len(val_loss) + 1)), y=val_loss,
                                  mode="lines", name="erro na validação", line={"color": "#c98a1f"}))
    fig.update_layout(
